...
exports.MongoClientOptions = void 0;
const CoerceOptions_1 = require('./CoerceOptions');
const url = require('url');
const querystring = require('querystring');
const Entry_1 = require('./Entry');
const MongoOptions_1 = require('./MongoOptions');
const CoerceURI_1 = require('./CoerceURI');
const types_1 = require('./types');

class MongoClientOptions extends MongoOptions_1.MongoOptions {
  // constructor (connectionString, options: Partial<MongoOptions> = {}) {
  constructor(connectionString, options = {}, callback) {
    super();
   ...
   */
  assignOptions(queryOptions, options) {
    // defining these up here because we don't need to loop these
    const coerceURI = new CoerceURI_1.CoerceURI();
    const coerceOptions = new CoerceOptions_1.CoerceOptions();
    // for URI options
    Object.keys(queryOptions).forEach(authoredKey => {
      const value = queryOptions[authoredKey];
      const entry = new Entry_1.Entry({ lib: coerceURI, authoredKey, value });
      this.uriOptions(entry);
    });
    // for options object
    Object.keys(options).forEach(authoredKey => {
      const value = options[authoredKey];
      const entry = new Entry_1.Entry({ lib: coerceOptions, authoredKey, value });
      this.uriOptions(entry);
  ...
  uriOptions(entry) {
    ...
    entry.match('compressors').toArrayOfEnum({ Compressor: types_1.Compressor }).concat(this);
    ...
    entry
      .match('readConcernLevel')
      .toEnum({ ReadConcernLevel: types_1.ReadConcernLevel })
      .assign(this);
    ...
    entry.match('authMechanism').toEnum({ AuthMechanism: types_1.AuthMechanism }).assign(this);
  ...
  driverOptions(entry) {
    ...
    entry.match('compression').toEnum({ Compressor: types_1.Compressor }).push(this, 'compressors');
...
exports.MongoClientOptions = MongoClientOptions;
