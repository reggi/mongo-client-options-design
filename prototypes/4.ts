...
class Coercion {
    ...
    static authMechRequireUser(value: any, options: any ) {
        const mechanisms = [
            'GSSAPI',
            'MONGODB-CR',
            'PLAIN',
        ...
        const requiresUser = value && mechanisms.indexOf(value) === -1
        const user = MongoOptions.auth.user(options)
        if (requiresUser && !user) {
            throw new Error(`Username required for mechanism \`${value}\``);
        }
        return value
    }

    static userRequiredAuthMech(value: any, options: any ) {
        const mechanisms = [
            'GSSAPI',
            'MONGODB-CR',
            'PLAIN',
        ...
        const authMech = MongoOptions.authMechanism(options)
        const requiresUser = authMech && mechanisms.indexOf(authMech) === -1
        if (requiresUser && !value) {
            throw new Error(`Username required for mechanism \`${authMech}\``);
        }
        return value
    }
}
...
class MongoOptions {
    ...
    static authMechanism(value?: string | boolean, options?: any): boolean | undefined {
        return MongoOption
            ...
            // .handle(value => {
            //     Coercion.authMechanismRequirements(value, [
            //         MongoOptions.auth.user(value, options)
            //     ])
            // })
    }
